...
def fetch_data(token, dt_inicio, dt_fim):
    url = "https://api.2clix.com.br/v3/Reports/ExtracaoBaseResumida"
    params = {
        "DtInicio": dt_inicio,
        "DtFim": dt_fim,
        "codTipoFicha": 1
...
def insert_data(data):
    ...
    for item in data:
        ...
        query = "SELECT COUNT(*) FROM ofensores2clixapi WHERE CODIGO_AVALIACAO = ?"
...
def remove_duplicates():
    ...
    try:
        cursor.execute("exec sp_RemoveDuplicatedData")
        conn.commit()
...
def main():
    try:
        ...
        dt_fim = "2024-05-31"  
        data = fetch_data(token, dt_inicio, dt_fim)
        insert_data(data)
        ...
        print(f"Erro a tirar duplicados: {e}")
